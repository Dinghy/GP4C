function [mPhi,DmPhidx] = MatrixPhi(V, model, TBeg, TEnd)
...
D = size(model.Xm,2); % dimension of points
...
if nargout==2
    if iscell(model.GP.logtheta)
        ...
        DmPhidx = cell(1,length(model.GP.logtheta));
        for d=1:length(model.GP.logtheta)
            DmPhidx{d} = zeros(M);
        end
    end
end
...
for d=1:D
    ...
    mSqQ = mQ.^2/4/nAlpha;
    mExpQ = exp(-mSqQ);
    mZbarMax=mZbar-TEnd(d)/nAlphaSqrt;
    mZbarMin=mZbar-TBeg(d)/nAlphaSqrt;
...
if nargout==2
    if D==2
        ...
        DmPhidx{3} = 4*nGamma^2*mPhi;
    else
        DmPhidx{1} = nGamma^2*(mPhi_save{1}.*mTmp_a{d}+mTmp_b{d});
        DmPhidx{2} = 4*nGamma^2*mPhi;
    end
end
mPhi=nGamma^2*mPhi;
end
